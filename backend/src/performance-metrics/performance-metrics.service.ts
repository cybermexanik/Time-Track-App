...
export class PerformanceMetricsService {
  ...
  async create(createPerformanceMetricDto: CreatePerformanceMetricDto, id: number) {
    const currentPerfomanceMetric = await this.perfomanceMetricsRepository.findBy({
      user: {id},
  ...
  async update(id: number, updatePerformanceMetricDto: UpdatePerformanceMetricDto) {
    const metric = await this.perfomanceMetricsRepository.findOne({
      where:{id},
    })

    if(!metric) throw new NotFoundException('Метрик не найден')
    return await this.perfomanceMetricsRepository.update(id, updatePerformanceMetricDto)
  }
